...
function ToggleGroup({ options, value, onChange, testId }: {
  ...
  return (
    <div style={{ display: 'flex', borderRadius: 4, overflow: 'hidden', border: `1px solid ${theme.color.border}`, flexShrink: 0 }}>
      {options.map((opt, i) => (
        <button
          key={opt.value}
          ...
          style={{
            ...
            borderLeft: i > 0 ? `1px solid ${theme.color.border}` : 'none',
            cursor: 'pointer', fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap',
            background: value === opt.value ? P.btnBlue : 'transparent',
            color: value === opt.value ? '#fff' : theme.color.textBody,
          }}
        >{opt.label}</button>
      ))}
    </div>
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function DevLayout({ watershed }: { watershed: 'ping' | 'yom' }) {
  ...
  useEffect(() => {
    if (!dateStripRef.current || !selectedDate) return;
    const el = dateStripRef.current.querySelector(`[data-date="${selectedDate}"]`) as HTMLElement | null;
    el?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
  }, [selectedDate]);

  // mouse-wheel scrolls date strip horizontally
  useEffect(() => {
  ...
  const handleSubModeChange = async (sub: 'aggregate' | 'daily') => {
    ...
    if (viewMode === 'basin') {
      const url = sub === 'daily' ? `${API}/basin/dates?model=${model}&mb_code=${mbCode}&sub=daily` : `${API}/basin/dates?model=${model}&mb_code=${mbCode}`;
      ...
      const url = sub === 'daily' ? `${API}/forecast/dates?model=${model}&mb_code=${mbCode}&sub=daily` : `${API}/forecast/dates?model=${model}&mb_code=${mbCode}`;
      const dates = await fetch(url).then(r => r.json());
      const vd = Array.isArray(dates) ? dates : [];
      const latest = vd[vd.length - 1] ?? '';
      setAvailableDates(vd);
  ...
  const handleViewModeChange = async (m: 'admin' | 'basin') => {
    ...
    if (m === 'basin') {
      ...
      const url = subMode === 'daily' ? `${API}/forecast/dates?model=${model}&mb_code=${mbCode}&sub=daily` : `${API}/forecast/dates?model=${model}&mb_code=${mbCode}`;
      const dates = await fetch(url).then(r => r.json());
      const vd = Array.isArray(dates) ? dates : [];
      const date = vd.includes(selectedDate) ? selectedDate : (vd[vd.length - 1] ?? '');
      setAvailableDates(vd);
      ...
      <div className="fc-main">
        ...
        >
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            {/* Logo + title */}
            <div style={{ padding: '10px 10px 8px', flexShrink: 0, borderBottom: `1px solid ${P.sidebarBorder}` }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: theme.fontSize.xs, color: theme.color.textLabel, flex: 1 }}>{t.sidebar.exportData}</span>
        ...
        <div className="proto-right">
          ...
          <div style={{
            ...
            <ToggleGroup
              ...
              value={viewMode}
              onChange={v => handleViewModeChange(v as 'admin' | 'basin')}
              testId="viewmode-dropdown"
            />
            <ToggleGroup
              options={modelOptions}
              value={model}
              onChange={v => handleModelChange(v as Model)}
            />
            <ToggleGroup
              options={subModeOptions}
              value={subMode}
              onChange={v => handleSubModeChange(v as 'aggregate' | 'daily')}
            />
          ...
          {/* ── Control bar: mode pills + date strip ─────────────────────────── */}
          <div style={{
            display: 'flex', alignItems: 'center', gap: 0,
            background: P.sectionBg, borderBottom: `1px solid ${P.topBarBorder}`,
            flexShrink: 0, overflow: 'hidden',
          }}>
            {/* Mode pills */}
            <div style={{ display: 'flex', flexShrink: 0, borderRight: `1px solid ${theme.color.border}` }}>
              {modeOptions.map(opt => (
                <button
                  key={opt.value}
                  ...
                  style={{
                    ...
                    fontSize: 12, fontWeight: mode === opt.value ? 700 : 400,
                    background: mode === opt.value ? '#fff' : 'transparent',
                    color: mode === opt.value ? theme.color.textPrimary : theme.color.textLabel,
                    borderBottom: mode === opt.value ? `2px solid ${MODE_DOT[opt.value as Mode]}` : '2px solid transparent',
                    transition: 'background 0.15s',
                  }}
                >
                  <span style={{
                    width: 9, height: 9, borderRadius: '50%', flexShrink: 0,
                    background: MODE_DOT[opt.value as Mode],
            ...
            {/* Date strip with nav buttons */}
            <div style={{ display: 'flex', alignItems: 'center', flex: 1, overflow: 'hidden', padding: '4px 6px', gap: 4 }}>
              <button
                onClick={() => { if (dateStripRef.current) dateStripRef.current.scrollLeft -= 200; }}
                style={{
                  flexShrink: 0, width: 24, height: 26, border: 'none',
                  borderRadius: theme.radius.md, background: P.btnBlue,
              ...
              <div
                ref={dateStripRef}
                style={{ display: 'flex', gap: 4, overflowX: 'auto', flex: 1, scrollbarWidth: 'none' }}
              >
                {availableDates.length === 0 ? (
                  <span style={{ fontSize: 12, color: theme.color.textMuted, padding: '2px 4px' }}>—</span>
                ) : (
                  availableDates.map(d => (
                    <button
                      key={d}
                      data-date={d}
                      data-testid={`date-dropdown-option-${d}`}
                      onClick={() => handleDateSelect(d)}
                      style={{
                        padding: '3px 10px', border: `1px solid ${d === selectedDate ? P.btnBlue : theme.color.border}`,
                        borderRadius: 20, cursor: 'pointer', whiteSpace: 'nowrap',
                        fontSize: 11, fontWeight: d === selectedDate ? 700 : 400,
                        background: d === selectedDate ? P.btnBlue : '#fff',
                        color: d === selectedDate ? '#fff' : theme.color.textBody,
